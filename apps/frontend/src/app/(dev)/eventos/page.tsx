            ...
            {eventos.map((evento) => (
                <div key={evento.id} className="flex flex-col w-full bg-zinc-800 rounded-lg overflow-hidden">
                    <div className="relative w-full h-52">
                        <Image className="object-cover" src={evento.imagem} fill alt={evento.nome} />
                    </div>
                    <div className="flex-1 flex flex-col items-center p-7 gap-5 text-center">
                        ...
                        <QRCode value={JSON.stringify({ id: evento.id, senha: evento.senha})} className="w-44 h-44"></QRCode>
                    </div>
                    <div>
